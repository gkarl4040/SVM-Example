...
# Date: 06/20/2020
# Description: This script uses sklearn's SVM library to classify
#              abalone as young (< 11 rings) or old (>= 11 rings)
#              based on measurements of their physical characteristics.
#
...
param_list = [{'kernel': ['rbf'], 'gamma': [1e-2, 1e-3], 'C': [2, 20, 200, 2000]},
              {'kernel': ['linear'], 'C': [2, 20, 200, 2000]}]

performance_metrics = ['precision', 'recall']

# Output performance metrics for optimal hyperparams and kernel choices
for metric in performance_metrics:
    ...
    # Perform grid search to find optimal hyperparams and kernels
    clf = GridSearchCV(SVC(), param_list, scoring='%s_macro' % metrics)
